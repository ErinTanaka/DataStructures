...
void _dynArrSetCapacity(DynArr *v, int newCap)
{
	...
	//Part 1 conditional statements determining difference between shrinking and expanding capacity so memory that shouldn't be accessed isn't overwritten
	
	//Part 1 first conditional statement, capacity has been doubled
	if (newCap > oldCapacity) {
		for (i = 0; i < oldCapacity; i++) {
			v->data[i] = oldData[i];
		}
	}
	//Part 1 second conditional statement, capacity has been halved
	else {
		for (i = 0; i < newCap; i++) {
			v->data[i] = oldData[i];
		}
...
void removeAtDynArr(DynArr *v, int idx){
   ...
   for(i = idx; i < v->size-1; i++){
	  ...
	  //Part  1 shrinking the array if its size is less than 1/3 of the capacity
      if(v->size < v->capacity/3){ //check to see if "resize" is neccessary
	 _dynArrSetCapacity(v, v->capacity/2); //call capacity changing function
...
void popDynArr(DynArr *v)
{
	assert(v!=0);
	assert(! isEmptyDynArr(v));
	//Part 1 Removing/commenting out the direct accessing of the struct element size.
	//v->size--;
	//Part 1 using function removeAtDynArr to take "top" element off of the stack
