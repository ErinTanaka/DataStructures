...
struct data * findNode(struct Node *tree, int val){
  ...
  */
	...
	while (cur != NULL){
		if (cur->val->number == val) {//val found
			return cur->val;
		}
		else {
...
int countNodes(struct Node *tree){
  /*  Returns the number of nodes present in tree including tree
	...
	else return (1 + countNodes(tree->right) + countNodes(tree->left)); // current node+number of nodes in left subtree+number of nodes in right subtree
...
int secondLargestNode(struct Node *tree){
  ...
  */
	...
	if(tree->right == NULL && tree->left == NULL) return -1; // tree has less than 2 nodes
	
	struct Node * largest=tree; 
	struct Node * sec;

	while (largest->right != NULL) { // find largest node in tree
		largest = largest->right;
	}
	if (largest->left != NULL) { // largest node has a subtree
		sec = largest->left;
		while (sec->right != NULL) { //find largest node in left subtree
			sec = sec->right;
		}
		return sec->val->number;
	}
	else {//largest node doesn't have a left subtree
		sec = tree;
		while (sec->right != largest) { // find parent of largest node
			sec = sec->right;
		}
		return sec->val->number;
	}
...
void  flattenPreorder(struct Node *tree, struct cirListDeque * d){
  /* Performs a preorder traversal of tree and appends nodes in the
	...
	if (tree == NULL) { // nothing to process
		return;
	}
	else {
	    addBackCirListDeque(d,tree->val); // add value to deque
		flattenPreorder(tree->left, d); // process left subtree
		flattenPreorder(tree->right, d); // process right subtree
	}
...
//	printf("post order\n");
	if (tree == NULL) { // nothing to process
		...
		flattenPostorder(tree->left, d); // process left subtree
		flattenPostorder(tree->right, d); // process right subtree
	    addBackCirListDeque(d,tree->val); // add value to deque
	}
  return;
}
...
//	printf("inorder\n");
	if (tree == NULL) { // nothing to process
		...
		flattenInorder(tree->left, d); // process left subtree
	    addBackCirListDeque(d,tree->val); // add value to deque
		flattenInorder(tree->right, d); // process right subtree
	}
	return;
}
/*----------------------------------------------------------------------------*/
